...
struct fp_dscv_print **dscv_prints = NULL;
...
int main(void)
{
    ...
    dscv_prints = fp_discover_prints();
   
    if (!dscv_prints) {
		fprintf(stderr, "Error loading enrolled prints.");
        goto out;
	}
	...
	i = 0;
	while ((print = dscv_prints[i++])) {
		int fnum;
		
		//Determines if a fp_dscv_print discovered print 
		//appears to be compatible with a certain device.
		if (!fp_dev_supports_dscv_print(dev, print))
			continue;
		
		// menentukan jari apa yang terekam di disk
